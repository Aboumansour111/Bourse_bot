...
def build_breadth_map(conn):
    """
    Build historical market breadth once.

    For each symbol:
      current close is compared with its immediately previous
      available close.

    Then aggregate positive/negative/unchanged by trade date.
    """
    ...
    for inscode, trade_date, close_price in rows:
        close_price = float(close_price)
        previous = previous_by_symbol.get(inscode)

        if previous is not None:
            if close_price > previous:
                counts[trade_date][0] += 1
            elif close_price < previous:
                counts[trade_date][1] += 1
            else:
                counts[trade_date][2] += 1

        previous_by_symbol[inscode] = close_price

    breadth = {}

    for trade_date, (positive, negative, unchanged) in counts.items():
...
def main():
    ...
    print("Max holding period:", MAX_HOLD_DAYS, flush=True)
    print("Round-trip cost:", ROUND_TRIP_COST, flush=True)
    print("Building historical breadth map...", flush=True)

    breadth_map = build_breadth_map(conn)
    ...
    for symbol_index, (inscode, symbol) in enumerate(symbols, 1):
        ...
        # TSETMC may return 0/missing High/Low.
        # For ATR/stop calculations, replace invalid values with Close.
        highs = [
            float(row[2])
        ...
        max_i = len(rows) - MAX_HOLD_DAYS

        for i in range(MIN_HISTORY, max_i):
            ...
            market = breadth_map.get(signal_date)
        ...
        if symbol_index % 25 == 0 or symbol_index == len(symbols):
            print(
    ...
    print()
    print("========== FINAL STRATEGY BACKTEST FAST ==========")
