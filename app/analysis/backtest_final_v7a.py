...
# V7: volatility-adjusted position sizing
VOL_TARGET_ATR_PCT = 0.04
MIN_VOL_MULTIPLIER = 0.50
MAX_VOL_MULTIPLIER = 1.25
...
def volatility_multiplier(entry, atr_value):
    if entry <= 0 or atr_value is None or atr_value <= 0:
        return 1.0
    ...
    multiplier = VOL_TARGET_ATR_PCT / atr_pct
...
def momentum_returns(closes):
    ...
    if n >= 121 and closes[-121] > 0:
        result["mom120"] = (
            closes[-1] / closes[-121] - 1.0
        )

    if n >= 253 and closes[-253] > 0:
        result["mom252"] = (
            closes[-1] / closes[-253] - 1.0
        )
...
def technical_signal(closes, highs, lows, volumes):
    ...
    momentum = momentum_returns(closes)
    ...
    return {
        ...
        "mom120": momentum["mom120"],
        "mom252": momentum["mom252"],
    }
...
def generate_candidates(data, breadth):
    candidates = []
    ...
    for symbol_index, (inscode, item) in enumerate(
        ...
        for i in range(
            ...
            candidates.append({
                ...
                "mom60": signal["mom60"],
                "mom120": signal["mom120"],
                "mom252": signal["mom252"],
            })
    ...
    # --------------------------------------------------------
    # V7: cross-sectional momentum ranking
    # Rank candidates only against other candidates
    # entering on the same date.
    # --------------------------------------------------------
    ...
    for date, group in candidates_by_date.items():

        # Primary: 120-day momentum.
        # Secondary: 60-day momentum.
        # Tertiary: 252-day momentum.
        group.sort(
            key=lambda x: (
                x["mom120"]
                if x["mom120"] is not None
                else -999.0,

                x["mom60"]
                if x["mom60"] is not None
                else -999.0,

                x["mom252"]
                if x["mom252"] is not None
                else -999.0,
        ...
        for rank, candidate in enumerate(group, 1):
            if total <= 1:
                candidate["momentum_rank"] = 1.0
            else:
                candidate["momentum_rank"] = (
                    (total - rank) / (total - 1)
                )
...
def build_price_map(data):
    ...
    for inscode, item in data.items():
        for date, close in zip(
            ...
            prices[(inscode, date)] = close

    return prices


def simulate_portfolio(candidates, data):
    candidates_by_date = defaultdict(list)
    ...
    # موقعیت‌های باقی‌مانده در انتهای داده
    for pos in positions:
        last_price = price_map.get(
            (pos["inscode"], pos["exit_date"]),
            pos["entry"],
        )
...
def print_results(
    ...
    avg_win = (
        mean(x["return_pct"] for x in winners)
        if winners else 0
    )

    avg_loss = (
        mean(x["return_pct"] for x in losers)
        if losers else 0
    )
    ...
    print()
    print("========== FINAL PORTFOLIO BACKTEST V2 ==========")
    print("Candidate trades:", len(candidates))
    print("Accepted trades:", len(trades))
    print("Initial capital:", f"{INITIAL_CAPITAL:,.0f}")
    print("Final equity:", f"{final_equity:,.0f}")
    print(f"Total return: {total_return:.2f}%")
    print(f"Win rate: {win_rate:.2f}%")
    print(f"Average return/trade: {average_return:.2f}%")
...
def main():
    ...
    print()
    print("Simulating capital-constrained portfolio...")
    ...
    if equity_curve:
        ...
        print("Simulation days:", len(equity_curve))
