...
# Trailing Stop test parameters
TRAILING_TRIGGER_ATR = float(__import__("os").environ.get("TRAILING_TRIGGER_ATR", "1.0"))
TRAILING_DISTANCE_ATR = float(__import__("os").environ.get("TRAILING_DISTANCE_ATR", "1.0"))
    ...
    for symbol_index, (inscode, item) in enumerate(
        ...
        for i in range(
            ...
            for j in range(
                ...
                # Activate trailing after price moves
                # TRAILING_TRIGGER_ATR above entry.
                trigger_price = (
                    entry
                    + TRAILING_TRIGGER_ATR * atr14
                )

                if (
                    not trailing_active
                    and highest_price >= trigger_price
                ):
                    trailing_active = True

                # Trail behind the highest price.
                if trailing_active:
                    trailing_stop = max(
                        trailing_stop,
                        highest_price
                        - TRAILING_DISTANCE_ATR * atr14
                    )

                hit_target = day_high >= target

                if trailing_active:
                    hit_stop = day_low <= trailing_stop
                ...
                if hit_stop and hit_target:
                    ...
                    if trailing_active:
                        exit_reason = "TRAILING_AND_TARGET_SAME_DAY"
                    else:
