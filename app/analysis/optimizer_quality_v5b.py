...
QUALITY_VALUES = [65, 70, 75, 80, 85, 90]
...
def run(pool, data, start, end, threshold):
    candidates = [
        x for x in pool
        if (
            start <= x["entry_date"] <= end
            and start <= x["exit_date"] <= end
            and x["quality"] >= threshold
        )
    ]
...
def main():
    ...
    print(
        "QUALITY | "
        "TRAIN_C | TRAIN_TR | TRAIN_RET | TRAIN_PF | TRAIN_DD | "
        "OOS_C | OOS_TR | OOS_RET | OOS_PF | OOS_DD"
    )

    for threshold in QUALITY_VALUES:

        train_candidates, train = run(
            pool,
            data,
            TRAIN_START,
            TRAIN_END,
        ...
        print(
            f"{threshold:>7} | "
            f"{len(train_candidates):>7} | "
            f"{train_trades:>8} | "
            f"{train_return:>9.2f}% | "
            f"{train_pf:>8.2f} | "
            f"{train_dd:>8.2f}% | "
            f"{len(oos_candidates):>5} | "
            f"{oos_trades:>6} | "
            f"{oos_return:>7.2f}% | "
            f"{oos_pf:>6.2f} | "
            f"{oos_dd:>7.2f}%"
