...
def technical_signal_volume_limit(
    closes,
    highs,
    lows,
    volumes,
    ...
    # Only technical conditions that are independent
    # of the volume lower threshold are applied here.
    eligible_base = (
        trend >= 4
...
def build_candidate_pool(data, breadth):
    ...
    print("Building volume candidate pool...")
    ...
    for symbol_index, (inscode, item) in enumerate(
        ...
        for i in range(
            ...
            signal = technical_signal_volume_limit(
                closes[:i + 1],
                highs[:i + 1],
                lows[:i + 1],
                volumes[:i + 1],
            ...
            if signal is None or not signal["eligible_base"]:
                continue
...
def main():
    ...
    for volume_low in VOLUME_LOW_VALUES:
