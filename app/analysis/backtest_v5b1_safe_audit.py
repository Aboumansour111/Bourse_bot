...
def atr(highs, lows, closes, gap_before=None, period=14):
    ...
    for i in range(1, len(closes)):
        ...
        # If the previous OHLC observation is invalid/missing,
        # do not use the stale previous close in True Range.
        if gap_before[i]:
            tr = highs[i] - lows[i]
        else:
            tr = max(
...
def simulate_portfolio(candidates, data):
    ...
    for trade in candidates:
        if 20260101 <= trade["entry_date"] <= 20261231:
            candidates_by_date[
                trade["entry_date"]
            ].append(trade)
    ...
    # موقعیت‌های باقی‌مانده در انتهای بازه تست
    # هیچ داده‌ای بعد از 2026 وارد بک‌تست نمی‌شود.
